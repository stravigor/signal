...
Stores notifications in `_stravigor_notifications` for in-app notification features (unread badges, notification feeds).
...
`NotificationManager.ensureTable()` creates `_stravigor_notifications`:
